...
import com.ccteam.graphql.config.security.AuthenticationErrorResponse;
...
import com.ccteam.graphql.config.security.LockoutPolicy;
 ...
 * <p>
 * Implements a per-account progressive lockout to mitigate brute-force
 * attempts on the 6-digit passcode. The thresholds and durations live
 * in {@link LockoutPolicy} so the curve is documented and tweakable in
 * one place. The counter and {@code lockedUntil} timestamp are stored
 * on the {@link Member} entity.
 *
 * @author yann39
 * @since 1.0.0
 */
...
public class AuthController {
     ...
     * Outcomes:
     * <ul>
     *   <li><b>423 Locked</b> — account is currently in a lockout window;
     *       body carries the number of seconds left before unlock.</li>
     *   <li><b>401 Unauthorized</b> — wrong passcode (or unknown email);
     *       body carries {@code attemptsLeft} when known so the client
     *       can warn the user before the next lockout kicks in.</li>
     *   <li><b>500 Internal Server Error</b> — unexpected auth failure
     *       (anonymous token, etc.). Should not happen under normal
     *       usage but kept as a safety net.</li>
    ...
    public ResponseEntity<?> authenticate(@RequestBody JWTRequest userRequest) {
        ...
        // 1) pre-check: is the account currently locked?
        // We do this BEFORE the BCrypt comparison so a flood of
        // attempts can't keep the CPU busy hashing during a lockout.
        // We don't reveal "this account doesn't exist" vs "this
        // account is locked" — an unknown email just goes through
        // the normal flow and ends up on a 401 with no attempt info.
        ...
        // 2) verify the passcode via Spring Security's AuthenticationManager.
        ...
        try {
            ...
            // 3) wrong passcode → record the failure and apply the
            // progressive lockout policy. We only do this when the
            // email actually exists in the database — for unknown
            // emails we return a plain 401 with no attempts info so
            // we don't leak whether the email is registered.
